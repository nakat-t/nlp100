...
fn main() -> io::Result<()> {
    ...
    let v: Vec<f64> = map.values().map(|x| *x as f64).collect();

    let h = Histogram::from_slice(&v, Bins::Count(100));
    let view = ContinuousView::new().add(&h).y_range(0.0, 200.0);
