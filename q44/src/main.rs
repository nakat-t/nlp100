...
fn make_dot(sentence: &Vec<Chunk>) -> String {
    let mut s = String::new();
    s.push_str("digraph g {\n");
    s.push_str("\tnode [ fontname = \"Meiryo\" ];\n");
    for i in 0..sentence.len() {
        let src = sentence[i].phrase_without_marks();
        let dst = match sentence[i].dst {
            Some(d) => sentence[d].phrase_without_marks(),
            None => String::new(),
        };
        if src != "" && dst != "" {
            s.push_str(&format!("\t\"{}\" -> \"{}\";\n", src, dst));
...
fn main() {
    ...
    let s = match read_sentence_from_cabocha_lattice(&mut r) {
        Ok((n, _)) if n == 0 => panic!("failed to read cabocha lattice."),
        Ok((_, s)) => s,
        Err(_) => panic!("failed to read cabocha lattice."),
    };
    println!("{}", make_dot(&s));
}
